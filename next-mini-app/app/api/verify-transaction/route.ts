...
export async function POST(request: Request) {
  try {
    ...
    // MÉTODO 1: Verificar con Alchemy (como el sistema de CDT)
    try {
    ...
    // MÉTODO 2: Verificar con WorldScan (respaldo)
    try {
      console.log("Verificando con WorldScan...")
    ...
    // MÉTODO 3: Verificar si la página existe (último recurso)
    try {
      console.log("Verificando si la página existe...")
      ...
      if (checkResponse.ok) {
        console.log("La página de la transacción existe")
        return NextResponse.json({
          success: true,
          isValid: true,
          method: "page_exists",
      ...
      console.error("Error verificando página:", pageError)
    }
