...
export function BoostModal({
  ...
  const handlePurchase = async () => {
    try {
      ...
      // WALLET CORREGIDA: Usar la wallet correcta
      ...
      // VERIFICACIÓN INICIAL: Si no hay success, ni siquiera intentamos
      ...
      // VERIFICACIÓN BLOCKCHAIN: Usando nuestro propio endpoint
      setVerifyingTransaction(true)
      setIsLoading(false)

      // Esperar 5 segundos para que la transacción se propague
      await new Promise((resolve) => setTimeout(resolve, 5000))

      // Verificar la transacción usando nuestro endpoint
      ...
      // Si la transacción es válida, crear el boost
      if (verifyData.success && verifyData.isValid) {
        ...
        setError("La transacción no pudo ser verificada en blockchain")
      }
    } catch (error) {
